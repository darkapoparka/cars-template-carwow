# Carwow-style — AI operating instructions

This repository is the canonical reusable master for the **`carwow`** automotive template. Treat these instructions as the governing contract for AI work in this repo.
...
Before declaring a lead ready, search the entire copy (including metadata and assets) for old identity markers. Start with: `DAY NIGHT AUTO GROUP|Day Night Auto|Day & Night|daynight|day-night|0877 733 110|Атанас Манчев|kristiankirilov`. Treat matches as review items, not blind replacements.
...
- When this template is inserted into a multi-variant dealer repo, it belongs in a variant folder such as `carwow/` with no nested `.git` directory.
