	...
	export const buttonVariants = tv({
		...
		variants: {
			...
			size: {
				...
				sm: "h-9 gap-1 rounded-[min(var(--radius-md),12px)] px-3 text-[length:var(--sa-text-caption)] in-data-[slot=button-group]:rounded-lg has-data-[icon=inline-end]:pr-2 has-data-[icon=inline-start]:pl-2 [&_svg:not([class*='size-'])]:size-3.5",
