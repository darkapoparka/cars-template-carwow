<script lang="ts">
	...
	function locallyDismissed() {
		try {
			return localStorage.getItem('cars.prompt.v1') === 'dismissed';
		} catch {
			return false;
		}
	}
	...
	async function submit(action: 'save' | 'dismiss') {
		...
		try {
			const response = await fetch(i18n.endpoint('/api/preferences'), {
				method: 'POST',
...
>
	<button
		...
		aria-label={i18n.t('locale.close')}
	...
	>
		...
		<p class="cars-locale-unavailable">{i18n.t('locale.moreLanguages')}</p>
		...
		<div class="cars-locale-actions">
			...
			<button type="submit" disabled={busy || !isLocale(locale)}
				>{i18n.t(busy ? 'locale.saving' : 'locale.save')}</button
			>
		</div>
	</form>
</dialog>
