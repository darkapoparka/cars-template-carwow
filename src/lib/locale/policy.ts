const hasUnsafeUrlCharacters = (value: string) =>
...
export function createLocaleRouting<const L extends Language>(input: LocaleConfiguration<L>) {
	...
	function isResource(pathname: string) {
		...
		return (
			/^\/(?:admin|api|_app|_next|assets|dealer-brand|dealer-inventory|brand|images|fonts|ingest)(?:\/|$)/.test(
				path
	...
	function localeHref(href: string, locale: L, defaultBase = ''): string {
		...
		if (!href.startsWith('/') || href.startsWith('//') || hasUnsafeUrlCharacters(href)) return href;
